...
class TestAbsLoss(unittest.TestCase):
    def test_simple(self):
        ...
        array1 = t.array([4, -2, 3, -4, 5, -6, 7, 2, 9])
        # load address of `array1` into register a1
        t.input_array("a1", array1)
        # set a2 to the length of the array
        t.input_scalar("a2", 9)
        # create a result array in the data section (fill values with -1)
        ouptut_array = t.array([-1]*len(array0))
        result_array = [3, 0, 0, 4, 0, 0, 0, 10, 0]

        # load address of `array2` into register a3
        t.input_array("a3", ouptut_array)
        # call the `abs_loss` function
        t.call("abs_loss")
        # check that the result array contains the correct output
        t.check_array(ouptut_array, result_array)
...
class TestSquaredLoss(unittest.TestCase):
    def test_simple(self):
        ...
        array0 = t.array([1, 3, 4, 9])
        array1 = t.array([1, 2, 3, 7])
...
class TestInitializeZero(unittest.TestCase):
    def test_simple(self):
        ...
        output_array = [0] * 10
        t.check_array_pointer("a0", output_array)
        t.execute()
